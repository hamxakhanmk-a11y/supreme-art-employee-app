...
(function (root) {
  ...
  function tableRows(model, start = 0, end = model.groups.length, checks = false) {
    return '<tbody>' + model.rows.map(r => '<tr>' +
      ...
      '<td>' + esc(r.name) + '</td><td>' + esc(r.unit) + '</td><td>' + display(start ? r.values[start-1].balance : r.opening) + '</td>' +
  ...
  const css = '.movement-table{border-collapse:collapse;width:100%;font:11px Arial,sans-serif}.movement-table th,.movement-table td{border:1px solid #bbb;padding:4px 6px;white-space:nowrap;text-align:right}.movement-table th{background:#f3eee4;text-align:center}.movement-table td:first-child{text-align:left}.movement-table small{display:block;font-size:9px;font-weight:normal}.movement-table .incoming{color:#15803d}.movement-table .outgoing{color:#a32d2d}.movement-table tbody tr{transform:none!important;box-shadow:none!important;break-inside:avoid}.movement-table thead{display:table-header-group}';
  ...
  api.output = function (kind) {
    ...
    win.document.write('<!doctype html><html><head><title>' + esc(title) + '</title><style>' + css +
      '@page{size:A4 landscape;margin:6mm}body{margin:0;color:#111}header{display:flex;align-items:center;background:#f5f5f5;border:1px solid #222;padding:3px 6px;gap:12px;break-inside:avoid;print-color-adjust:exact}.logo{width:165px;text-align:center;color:#a32d2d;font:bold 10px Arial}.logo img{display:block;width:100px;height:49px;object-fit:contain;margin:auto}.heading{flex:1;text-align:center}.heading h1{font: bold 15px Georgia;color:#a32d2d;margin:3px}.heading p,.note{font:9px Arial;margin:5px}section{break-after:page}section:last-child{break-after:auto}.movement-table{font-size:9px}.movement-table th,.movement-table td{padding:3px;white-space:normal}.movement-table small{font-size:8px}</style></head><body>' + pages + '</body></html>');
    win.document.close();
